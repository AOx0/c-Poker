...
Puntos calcularPuntos(Mano mano) {
    ...
    if (esEscaleraDeColor(mano)) return EscaleraDeColor;
    ...
    if (esDoblePareja(mano)) return DoblePareja;
...
Mano crearMano(Carta cartas[5]) {
    ...
    Mano mano = {
            ...
            &cartas[3],
            &cartas[4],
        .sonMismaFigura = cartas[0].figura == cartas[1].figura == cartas[2].figura== cartas[3].figura==cartas[4].figura,
        .masAlta =  cartas[0]
    };
...
int esEscaleraReal(Mano mano) {
    ...
    if ( mano.sonMismaFigura && containsValues(mano, 5, valores, 0, 0 ) ) return 1;
    return 0;
}
...
int esColor(Mano mano) {
    return mano.sonMismaFigura;
}

int esEscalera(Mano mano) {
    for (int i=2; i<=14; i++){
        Valor valores[] = {(Valor)i, (Valor)(i+1), (Valor)(i+2), (Valor)(i+3), (Valor)(i+4)};
        if (containsValues(mano, 5, valores, 0, 0) ) return 1;
    }
    return 0;
}
